import Options from '../../common/options';
declare const DeferredTransaction: {
    /** Capture a deferred transaction.
     *
     * Your commerce code must be configured to support deferred capture.
     *
     * @param token Unique transaction identifier
     * @param buyOrder Transaction's buy order
     * @param authorizationCode Transaction's authorization code
     * @param amount Amount to be captured
     * @param options (Optional) You can pass options to use a custom configuration for this request.
     */
    capture: (token: string, buyOrder: string, authorizationCode: string, amount: number, options?: Options) => Promise<any>;
    create: (buyOrder: string, sessionId: string, amount: number, returnUrl: string, options?: Options) => Promise<any>;
    commit: (token: string, options?: Options) => Promise<any>;
    status: (token: string, options?: Options) => Promise<any>;
    refund: (token: string, amount: number, options?: Options) => Promise<any>;
};
export default DeferredTransaction;
