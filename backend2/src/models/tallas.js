...
TallasModel.insert = (data, callback) => {
    pool.getConnection(async (err, cnn) => {
        ...
        try{
            ...
            await insertarTallasSubcategorias(cnn, result[0].insertId, data.sub_categorias)
...
TallasModel.update = (id, data, callback) => {
    pool.getConnection(async (err, cnn) => {
        ...
        try{
            ...
            await eliminarTallasSubcategorias(cnn, id, data.sub_categorias)
...
const insertarTallasSubcategorias = async (cnn, idTalla, arraySubCatId) => {
    await arraySubCatId.forEach(async e => {
        let res = await cnn.promise().query(`
        INSERT INTO tallas_subcategorias (
            tallas_id, 
            sub_categorias_id, 
            created_at, 
            updated_at
...
const eliminarTallasSubcategorias = async (cnn, idTalla, arraySubCatId) => {
    let res = await cnn.promise().query(`
        DELETE FROM tallas_subcategorias 
        WHERE 
            tallas_id = ${idTalla} AND 
            sub_categorias_id NOT IN (${arraySubCatId})
        `)
